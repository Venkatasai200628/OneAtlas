...
function AppPreview({ output, onModifyApp, onBackToGenerate, isExampleBlueprint = false }) {
  ...
  const rawAppName = schemas?.ui_schema?.design_system?.app_name || intent?.app_name || 'OneAtlas Studio';
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
            ...
            >
              ...
              {requireLogin && !isLoggedIn ? (
                <div ref={loginSectionRef} className="flex-1 min-h-0 overflow-y-auto bg-slate-50 dark:bg-slate-900 flex items-start justify-center p-6 sm:p-8 select-none animate-in fade-in duration-200">
                  <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-xl space-y-6">
                    {/* Brand Header */}
                    ...
                    <form onSubmit={handleMockLogin} className="space-y-4">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full text-xs px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                          required
                        />
                      </div>
                      <div className="space-y-1">
                      ...
                      <>
                        {hasEmailAuth && (
                          <div className="relative flex items-center justify-center">
                            ...
                            <span className="relative px-2 text-[10px] bg-white dark:bg-slate-900 text-slate-400 font-bold uppercase">or</span>
                          </div>
                        )}
                        <button
                          type="button"
                          onClick={() => { setIsLoggedIn(true); setLoginError(''); setActivePageIndex(0); }}
                          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                        >
                    ...
                    {hasEmailAuth && (
                      <div className="p-3 bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 rounded-xl text-center">
                        <p className="text-[10px] text-indigo-600 dark:text-indigo-400 font-semibold leading-normal">
                ...
                <div className="flex-1 flex overflow-hidden min-h-0">
                  ...
                  <aside className="w-52 bg-slate-50 dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col justify-between flex-shrink-0 select-none">
                    <div className="flex-1 flex flex-col">
                      ...
                      <div
                        ...
                        className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2.5 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                  ...
                  {/* Main Content Pane */}
                  <main className="flex-1 overflow-auto bg-slate-50/50 dark:bg-slate-950 flex flex-col">
                    {/* Page header banner */}
                    <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 flex items-center justify-between flex-shrink-0 select-none">
                      <div>
                        <h1 className="font-bold text-base text-slate-900 dark:text-white">{activePage?.name || 'Dashboard'}</h1>
                        <p className="text-xs text-slate-500 dark:text-slate-500 mt-0.5">Customized layouts for {appName}</p>
                      </div>
                      <span className="text-[9px] font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-slate-500 border border-slate-200/50 dark:border-slate-700/50">
                        ...
                        <div className="space-y-5">
                          {(pageComponents.find(c => c.type === 'settings')?.sections || [
                            ...
                            <div key={idx} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 p-5 shadow-sm space-y-3">
                              ...
                              <p className="text-[11px] text-slate-500 dark:text-slate-500">{section.description}</p>
                              ...
                              {section.title === 'Notifications' && (
                                <div className="space-y-2">
                                  {['Email digests', 'New record alerts', 'Weekly summary'].map(label => (
                                    <label key={label} className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-400">
                      ...
                      {isAdminAppPage && (
                        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm overflow-hidden">
                          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
                            ...
                            <p className="text-[11px] text-slate-500 mt-0.5">Only accounts that have signed in to OneAtlas</p>
                          </div>
                            ...
                            <table className="w-full text-left border-collapse text-xs">
                              ...
                              <tbody>
                                {(dbRecords[activeTableKey] || []).slice(0, 3).map((row, idx) => (
                                  <tr key={idx} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50/40 dark:hover:bg-slate-800/20 text-slate-600 dark:text-slate-400 font-medium">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="pl-7 pr-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 text-[11px] focus:outline-none focus:ring-1 focus:ring-indigo-400 focus:border-indigo-400 w-44"
                              />
                            </div>
                                ...
                                return (
                                  <div key={idx} className="bg-slate-100 dark:bg-slate-900 rounded-xl p-3 space-y-2 border border-slate-200/30 dark:border-slate-800 min-h-[260px] flex flex-col justify-start">
                                    <div className="flex items-center justify-between border-l-2 pl-2 mb-2 select-none border-indigo-500 leading-tight">
                                      ...
                                      <span className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-slate-200/70 dark:bg-slate-800 text-slate-500">{colDeals.length}</span>
                                    </div>
                                    <div className="space-y-2 overflow-y-auto flex-1 max-h-[220px]">
                                      {colDeals.map((c) => (
                                        <div key={c.id} className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border border-slate-200/60 dark:border-slate-700/60 shadow-xs space-y-1 hover:shadow-sm transition-all group relative">
                                          <p className="text-[11px] font-bold text-slate-900 dark:text-slate-200 leading-snug group-hover:text-indigo-500 transition-colors pr-4">{c.title}</p>
                                          <p className="text-[10px] text-slate-500 font-medium">{c.customer}</p>
                                          <p className="text-[10px] font-black text-indigo-500">${parseFloat(c.value).toLocaleString()}</p>
                                          <button
                                            onClick={() => handleDeleteRecord(c.id)}
                                            className="absolute right-1 bottom-1 text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 p-0.5 rounded transition-all"
                                      ...
                                      {colDeals.length === 0 && (
                                        <div className="text-[10px] text-slate-400 text-center py-6 border border-dashed border-slate-200 dark:border-slate-900 rounded-lg">Empty Column</div>
                                      )}
                                    </div>
                                  </div>
                                );
                            ...
                            <table className="w-full text-left border-collapse text-xs">
                              ...
                              <tbody>
                                {(dbRecords[activeTableKey] || [])
                                  ...
                                  .map((row, idx) => (
                                    <tr key={row.id || idx} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50/40 dark:hover:bg-slate-800/20 text-slate-600 dark:text-slate-400 font-medium animate-in slide-in-from-left-1 duration-150">
        ...
        {workspaceTab === 'dashboard' && (
          <div className="flex-1 min-h-[480px] flex overflow-hidden bg-white dark:bg-slate-950">
            ...
            <main ref={adminPanelRef} className="flex-1 overflow-auto bg-slate-50/40 dark:bg-slate-950 p-5 min-h-0">
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 p-5 shadow-sm flex items-start gap-4 hover:border-slate-300 dark:hover:border-slate-700 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        {editingName ? (
                          <div className="flex items-center gap-1.5 animate-in slide-in-from-top-1 duration-150">
                            ...
                            <button onClick={() => setEditingName(false)} className="px-2 py-1 rounded border text-[10px] text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800">Cancel</button>
                          </div>
                        ) : (
                          <div className="flex items-center gap-1.5">
                            <h2 className="font-extrabold text-base text-slate-900 dark:text-white leading-none">{appName}</h2>
                            <button
                              onClick={() => {
                                setTempAppName(appName);
                                setEditingName(true);
                              }}
                              className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-400 p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                      ...
                      <div className="flex items-center gap-2 mt-4 select-none">
                        <button onClick={() => setWorkspaceTab('preview')} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition-all border border-slate-200/40 dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 p-5 shadow-sm space-y-4">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer select-none group">
                        ...
                        />
                        <span className="text-xs font-semibold text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 p-5 shadow-sm space-y-4">
                      ...
                      <div className="flex gap-2 select-all">
                        <div className="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 rounded-xl px-3 py-2 flex items-center justify-between text-xs text-slate-500 font-medium truncate">
                          <span className="truncate">{userInviteLink}</span>
                          <button onClick={handleCopyLink} className="text-slate-400 hover:text-slate-600 p-0.5 rounded ml-1 transition-colors">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 p-5 shadow-sm space-y-4 col-span-2 select-none">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold border border-slate-200 dark:border-slate-700 rounded-xl transition-all">
                          Move App
                        </button>
                      </div>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm p-5 space-y-4 animate-in fade-in duration-200">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 select-none">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">All users who signed in to OneAtlas (live from your project)</p>
                    </div>
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse text-xs">
                      ...
                      <tbody>
                        {dashboardUsers.map((u) => (
                          <tr key={u.id} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50/40 dark:hover:bg-slate-800/20 text-slate-600 dark:text-slate-300 font-medium">
                    ...
                    if (rows.length === 0) {
                      return (
                        <div className="p-8 text-center bg-slate-50/50 dark:bg-slate-900/40 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
                          <p className="text-sm font-semibold text-slate-500">No data present</p>
                    ...
                    return (
                      <div className="overflow-x-auto border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs">
                        <table className="w-full text-left border-collapse text-xs">
                          ...
                          <tbody>
                            {rows.map((row, rIdx) => (
                              <tr key={row.id || rIdx} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/40 dark:hover:bg-slate-800/20 text-slate-600 dark:text-slate-400">
                                <td className="px-4 py-3 font-mono text-[10px] text-slate-400">{row.id}</td>
                                {cols.map(c => {
                                  ...
                                  return (
                                    <td key={c.name} className="px-4 py-3 max-w-[200px] truncate">
                                      {typeof val === 'boolean' ? (
                                        <span className={clsx('px-1.5 py-0.5 rounded text-[8px] font-extrabold uppercase', val ? 'bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-950/20' : 'bg-slate-200 text-slate-500 border border-slate-200')}>
                                          {val ? 'TRUE' : 'FALSE'}
                                        </span>
                                      ) : String(val !== undefined ? val : '')}
                                    </td>
                      ...
                      {section.title === 'Notifications' && (
                        <div className="space-y-2">
                          {['New user registered alerts', 'Real-time record update notifications', 'Daily analytical updates digests'].map(label => (
                            <label key={label} className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-400 cursor-pointer select-none">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm p-5 space-y-5 animate-in fade-in duration-200">
                  ...
                  <div className="flex gap-3">
                    <div className="flex-1 relative">
                      ...
                      <input
                        ...
                        className="w-full pl-16 pr-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-indigo-400"
                      />
                    </div>
                  ...
                  <div className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs">
                    <table className="w-full text-left border-collapse text-xs">
                      ...
                      <tbody>
                        <tr className="border-b border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400">
                          ...
                          <td className="px-4 py-3">
                            <span className="px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/30 text-[9px] font-extrabold uppercase tracking-wide">
                              🔐 SECURED BY SSL
                            </span>
                          </td>
                        </tr>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm p-5 space-y-4 animate-in fade-in duration-200">
                  ...
                  <div className="space-y-3 select-none">
                    {[
                      ...
                      return (
                      <div key={sec.key} className="flex items-center justify-between p-3.5 bg-slate-50/50 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-700 rounded-2xl">
              ...
              {adminActiveTab === 'Code' && (
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm p-5 space-y-4 animate-in fade-in duration-200">
                  ...
                  <div className="rounded-xl bg-slate-950 border border-slate-900 p-4 font-mono text-[10px] text-slate-400 leading-relaxed overflow-x-auto select-all">
...
app.listen(<span className="text-blue-400">3000</span>, () =&gt; console.log(<span className="text-emerald-400">'medcare server API active on port 3000'</span>));</pre>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200 select-none">
          <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
            {/* Title */}
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                    ...
                    <div key={col.name} className="space-y-1">
                      ...
                      {isBool ? (
                        <select
                          ...
                          className="w-full text-xs px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-400 cursor-pointer"
                        >
                        ...
                        <select
                          ...
                          className="w-full text-xs px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-400 cursor-pointer"
                        >
                        ...
                        <input
                          ...
                          value={newFields[col.name] || ''}
                          onChange={e => setNewFields(p => ({ ...p, [col.name]: e.target.value }))}
                          className="w-full text-xs px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                        />
                      ) : (
                        <input
                          type="text"
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-900 bg-slate-50 dark:bg-slate-900/60 flex-shrink-0 flex items-center gap-3">
        ...
        <div className="flex-1 relative">
          <input
            ...
            className="w-full pl-3 pr-9 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-400 placeholder:text-slate-400 transition-all"
          />
          <button
